...
Six slices complete and pushed to `main`. Current slice fixes Pro analysis routing and Pro upgrade visibility: the hero/recommendation Pro path now reaches the actual DeepSeek V4 Pro model, remains available after a Flash analysis, uses a larger Pro input budget for long documents, and replaces AI-derived Flash analysis fields instead of hiding the Pro result behind conservative merge rules.
